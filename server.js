...
app.post('/api/auth/register', async (req, res) => {
    try {
        ...
        await pool.query(
            ...
            [result.rows[0].id, JSON.stringify(initialState.resources), JSON.stringify(initialState.workshop), 
             JSON.stringify(initialState.owned_cars), JSON.stringify(initialState.races), JSON.stringify(initialState.race_history)]
        );
...
app.post('/api/game/state', authenticateToken, async (req, res) => {
    try {
        ...
        if (gameState.resources?.money?.value > 50000000) {
            return res.status(400).json({ error: 'Valori sospetti rilevati' });
        }
...
app.get('/api/game/leaderboard', async (req, res) => {
    try {
        const result = await pool.query(`
            SELECT 
                u.username,
                (gs.races->>'wins')::int as wins,
                (gs.races->>'completed')::int as completed,
                (gs.resources->'reputation'->>'value')::int as reputation
            FROM game_state gs
